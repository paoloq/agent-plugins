...
# A minified bundle is typically a very large file with no newlines in its
# leading chunk. Picking one as a benchmark target produces an unactionable
# task (no insertion point, no readable identifiers).
_MINIFIED_MIN_BYTES = 200_000
_MINIFIED_HEAD_BYTES = 4096
...
def _looks_generated(repo: Path, p: Path) -> bool:
    ...
    if parts & _GENERATED_DIR_PARTS:
        return True
    try:
        size = static_checks.safe_size(p)
        with p.open("r", encoding="utf-8", errors="replace") as fh:
            head = fh.read(_MINIFIED_HEAD_BYTES)
    except OSError:
        return False
    if size >= _MINIFIED_MIN_BYTES and "\n" not in head:
        return True
...
def _pick_targets(repo: Path) -> dict:
    ...
    locate = _pick_locate_target(repo, code_files, largest)
    ...
    return {
        ...
        "locate_concept": locate.get("concept") if locate else None,
        "locate_answer_path": locate.get("answer_path") if locate else None,
    }


# Symbol-definition regex for the Locate task: matches `class Foo`,
# `def foo`, `function foo`, `func foo`, `fn foo` across common languages.
_DEF_RE = _re.compile(
    r"^\s*(?:class|def|function|fn|func)\s+([A-Za-z_][A-Za-z0-9_]+)",
    _re.MULTILINE,
)
...
def _pick_locate_target(repo: Path, code_files: list[Path], avoid_path: str | None) -> dict | None:
    """Pick a symbol whose definition lives in exactly one file and which is
    referenced from at least one other file. The Locate task asks the
    subagent which file defines this symbol — the orchestrator grades the
    answer by string-matching against `answer_path`.

    Returns None when no suitable symbol exists (tiny repos, or every symbol
    is single-file). The orchestrator records `skipped: no candidate` for
    the Locate task in that case.
    """
    # def_sites[sym] -> set of relative paths defining the symbol
    ...
    for f in code_files:
        ...
        for m in _DEF_RE.finditer(text):
            sym = m.group(1)
            if len(sym) < 6 or _is_dunder(sym):
                continue
            # Skip generic / language-keyword-ish names that produce noisy
            # locate questions ("the file that defines `setup`").
            if sym.lower() in {"main", "setup", "init", "run", "test", "build",
                               "config", "handler", "handle", "execute", "start"}:
                continue
            def_sites.setdefault(sym, set()).add(rel)

    # Keep only symbols defined in exactly one file.
    unique = {sym: next(iter(paths)) for sym, paths in def_sites.items() if len(paths) == 1}
    if not unique:
        return None

    # Inverted index: token -> number of files containing it. Tokenize each
    # file once and aggregate; per-symbol lookup is then O(1). The previous
    # `sorted(..., key=usage_count)` approach ran a fresh regex over every
    # code file for every candidate symbol — multi-hour on large repos.
    token_files: dict[str, int] = {}
    for text in file_text.values():
        for tok in set(_TOKEN_RE.findall(text)):
            token_files[tok] = token_files.get(tok, 0) + 1

    # Cross-file count = total files with token, minus the owner file.
    counted = [
        (sym, owner, min(max(token_files.get(sym, 0) - 1, 0), 5))
        for sym, owner in unique.items()
    ]
    ranked = sorted(
        counted,
        key=lambda t: (-t[2], t[1] == (avoid_path or ""), -len(t[0])),
    )
    # Prefer a symbol with ≥1 cross-file usage and not in avoid_path. Fall
    # back to anything if the strict criterion finds nothing.
    for sym, owner, count in ranked:
        if count >= 1 and owner != avoid_path:
            return {
                "concept": f"the file that defines the symbol `{sym}`",
                "answer_path": owner,
            }
    sym, owner, _ = ranked[0]
    return {
        "concept": f"the file that defines the symbol `{sym}`",
        "answer_path": owner,
    }
